<template>
  <div class="wechat">
    <div class="header">
      ...
      <div class="text_con">绑定微信</div>
      <div class="icon_right">
      </div>
    </div>
    <div class="content">
      <div class="number">微信账号</div>
      <div class="text_bs">
        <input type="text" placeholder="请输入微信账号" v-model="num"/>
        <!-- <span>USDT</span> -->
      </div>
...
export default {
  ...
  methods: {
    init () {
      this.$post('/api/payment/list', { userId: this.ID, payType: 1 })
        .then(res => {
          if (res.data.page.records[0]) {
            this.wechat = res.data.page.records[0]
