    ...
    submit () {
      ...
      if (reg.test(this.newPassword) && this.newPassword.length >= 6) {
        if (this.newPassword === this.oldPassword) {
          if (this.verifyCode) {
            this.$post('/api/user/forgetPassword', {identifier: this.phone, newPassword: this.newPassword, userId: this.ID, verifyCode: this.verifyCode})
              .then(res => {
                if (res.data.code === 1) {
                  this.$message({
                    message: res.data.msg,
