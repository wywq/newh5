  ...
  <div class="sell">
    ...
    <div class="content">
      ...
      <div class="assers">
        ...
        <div class="name" style="color:rgba(234, 111, 90, 1);">{{ orderDetails.price }} CNY</div>
      </div>
      <!-- <div class="assers">
      ...
      <button class="btn" @click="submit()">卖出</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    submit () {
      // if (this.ID !== this.orderDetails.issueUserId) {
      this.$post('/api/dealNo/placeDeal', { sellUserId: this.ID, dealNoId: this.order_id, buyUserId: this.orderDetails.issueUserId, payPassword: this.password, userId: this.ID })
        .then(res => {
          if (res.data.code === 1) {
            this.$message({
