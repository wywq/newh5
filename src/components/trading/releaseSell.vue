<template>
  <div class="releaseSell">
    ...
    <div class="content">
      ...
      <!-- <div>{{ cn_new }}</div> -->
      <button class="btn" @click="submit()">发布卖出</button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    submit () {
      if (this.remarks && this.pay && this.num && this.password) {
        this.$post('/api/dealNo/add', { issueUserId: this.ID, dealType: 2, remarks: this.remarks, price: this.pay, coinNum: this.num, payPassword: this.password, userId: this.ID })
          .then(res => {
            if (res.data.code === 1) {
              this.$message({
