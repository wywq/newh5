  ...
  methods: {
    ...
    submit () {
      if (this.cuty_list.usdtOpenId) {
        if (this.usdtNum) {
          if (this.fio_code) {
            this.$post('/api/usdtTransfer/add', {iphone: this.phone, toSendAddr: this.cuty_list.usdtOpenId, amount: this.usdtNum, userId: this.ID, verifyCode: this.fio_code, addr: this.usdtAddr})
              .then(res => {
                if (res.data.code === 1) {
                  this.$message({
                    message: res.data.msg,
