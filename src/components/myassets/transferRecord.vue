  ...
  <div class="transferRecord">
    ...
    <div class="content" v-if="list.length > 0">
      <div class="new_list" v-for="(item,index) in list" :key="index">
        <div class="name">{{ item.billType == 1 ? '转入' : '转出' }} {{ item.cwf }} CWF</div>
        <div class="huti">
...
export default {
  ...
  methods: {
    init () {
      this.$post('/api/cwfLog/logs', {userId: this.ID, page: this.page})
        .then(res => {
          this.list = res.data.page.records
        })
      // this.$post('/api/cwfLog/list', {userId: this.ID, cwfType: '2'})
      //   .then(res => {
      //     this.list = res
      //   })
    },
